const botondeinicio = document.getElementById("botondeinicio");
    ...
    function textogameover() {
      ...
      mensajedeperder.style.fontWeight = 400;
      mensajedeperder.style.fontStyle = "normal";
      temporizador.style.display = "none"; // Oculta el temporizador
    }
    
    async function iniciarJuego() {
      const temporizador = document.getElementById("temporizador");
      temporizador.style.display = "block"; // Muestra el temporizador
    ...
    async function iniciarParticipante() {
      // Resto del código omitido por claridad
    }
    ...
    function iniciarTemporizador() {
      ...
      const temporizador = setInterval(() => {
        ...
        if (tiempoRestante <= 0) {
          ...
          textogameover(); // Cambiar el fondo a negro cuando expire el tiempo
          desaparecerboton();
          setTimeout(() => {
            window.close(); // Cierra la ventana del navegador
          }, 2000); // Espera 2 segundos antes de cerrar la ventana
          return;
        }

        temporizadorDisplay.innerText = tiempoRestante; // Actualizar el contenido del temporizador
